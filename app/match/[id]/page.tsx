...
async function getMatchDetail(id: string) {
  try {
    ...
    const res = await fetch(baseUrl + "/api/match-detail?id=" + id, { cache: "no-store" });
    ...
    return (
      <div style={{ minHeight:"100vh", backgroundColor:"#0d1117", color:"#e6edf3" }}>
        ...
        <div style={{ maxWidth:"900px", margin:"0 auto", padding:"80px 24px", textAlign:"center" }}>
          <h2 style={{ fontSize:"24px", fontWeight:"700" }}>Match not found</h2>
          <Link href="/" style={{ color:"#0d1117", backgroundColor:"#3fb950", padding:"10px 24px", borderRadius:"8px", textDecoration:"none", fontWeight:"600" }}>Back to Live Scores</Link>
        </div>
      </div>
    );
  }
  const isLive = match.matchStarted && !match.matchEnded;
  const t1 = match.teamInfo ? match.teamInfo[0] : null;
  const t2 = match.teamInfo ? match.teamInfo[1] : null;
  const team1 = match.teams ? match.teams[0] : "";
  const team2 = match.teams ? match.teams[1] : "";
  const waText = team1 + " vs " + team2 + " - " + (match.status || "") + " - Live: https://cricscore-xi.vercel.app/match/" + match.id;
  const waUrl = "https://wa.me/?text=" + encodeURIComponent(waText);
  return (
    <div style={{ minHeight:"100vh", backgroundColor:"#0d1117", color:"#e6edf3" }}>
      <Navbar />
      <div style={{ maxWidth:"900px", margin:"0 auto", padding:"32px 24px" }}>
        <Link href="/" style={{ display:"inline-flex", color:"#7d8590", textDecoration:"none", fontSize:"13px", marginBottom:"24px" }}>Back to Live Scores</Link>
        <div style={{ backgroundColor:"#161b22", border:"1px solid #30363d", borderRadius:"16px", padding:"28px", marginBottom:"28px" }}>
          <div style={{ display:"flex", gap:"8px", marginBottom:"20px", flexWrap:"wrap" }}>
            <span style={{ fontSize:"11px", color:"#d4a853", backgroundColor:"rgba(212,168,83,0.1)", padding:"3px 10px", borderRadius:"100px", border:"1px solid rgba(212,168,83,0.3)" }}>{match.matchType ? match.matchType.toUpperCase() : ""}</span>
            {isLive && <span style={{ fontSize:"11px", color:"#f85149", backgroundColor:"rgba(248,81,73,0.1)", padding:"3px 10px", borderRadius:"100px", border:"1px solid rgba(248,81,73,0.3)" }}>LIVE</span>}
            {match.matchEnded && <span style={{ fontSize:"11px", color:"#3fb950", backgroundColor:"rgba(63,185,80,0.1)", padding:"3px 10px", borderRadius:"100px", border:"1px solid rgba(63,185,80,0.3)" }}>COMPLETED</span>}
          </div>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:"16px", flexWrap:"wrap" }}>
            <div style={{ textAlign:"center", flex:1 }}>
              {t1 && t1.img && <img src={t1.img} alt={t1.name} style={{ width:"52px", height:"52px", borderRadius:"50%", marginBottom:"8px" }} />}
              <div style={{ color:"#e6edf3", fontWeight:"700", fontSize:"16px" }}>{team1}</div>
            </div>
            <div style={{ textAlign:"center", flex:2 }}>
              ...
              {match.score && match.score.length > 0 ? (
                ...
                <div style={{ color:"#7d8590", fontSize:"14px" }}>{match.matchStarted ? "In Progress" : "Yet to start"}</div>
              )}
            </div>
            <div style={{ textAlign:"center", flex:1 }}>
              {t2 && t2.img && <img src={t2.img} alt={t2.name} style={{ width:"52px", height:"52px", borderRadius:"50%", marginBottom:"8px" }} />}
              <div style={{ color:"#e6edf3", fontWeight:"700", fontSize:"16px" }}>{team2}</div>
            </div>
          </div>
          {match.status && (
          ...
          {match.venue && <div style={{ marginTop:"16px" }}><span style={{ color:"#7d8590", fontSize:"12px" }}>Venue: {match.venue}</span></div>}
          <div style={{ marginTop:"16px" }}>
            <a href={waUrl} target="_blank" rel="noopener noreferrer" style={{ display:"inline-flex", alignItems:"center", gap:"6px", backgroundColor:"rgba(37,211,102,0.1)", border:"1px solid rgba(37,211,102,0.3)", color:"#25d366", padding:"8px 16px", borderRadius:"8px", fontSize:"13px", fontWeight:"600", textDecoration:"none" }}>Share on WhatsApp</a>
          </div>
        </div>
                ...
                {inning.batsman && inning.batsman.length > 0 && (
                  <div>
                    <div style={{ display:"grid", gridTemplateColumns:"2fr 1fr 1fr 1fr 1fr", padding:"8px 20px", backgroundColor:"#0d1117" }}>
                      {["Batter","R","B","4s","6s"].map((h) => <div key={h} style={{ color:"#7d8590", fontSize:"11px", fontWeight:"600" }}>{h}</div>)}
                    </div>
                ...
                {inning.bowler && inning.bowler.length > 0 && (
                  <div>
                    <div style={{ display:"grid", gridTemplateColumns:"2fr 1fr 1fr 1fr 1fr", padding:"8px 20px", backgroundColor:"#0d1117", borderTop:"1px solid #30363d" }}>
                      {["Bowler","O","M","R","W"].map((h) => <div key={h} style={{ color:"#7d8590", fontSize:"11px", fontWeight:"600" }}>{h}</div>)}
                    </div>
