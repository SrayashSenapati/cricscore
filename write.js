...
export default async function MatchDetail({ params }) {
  ...
  const shareText = encodeURIComponent((match.teams ? match.teams[0] : "") + " vs " + (match.teams ? match.teams[1] : "") + " - " + (match.status || "") + " - Live on CricScore: https://cricscore-xi.vercel.app/match/" + match.id);
      ...
      <div style={{ maxWidth: "900px", margin: "0 auto", padding: "32px 24px" }}>
        ...
        <div style={{ backgroundColor: "#161b22", border: "1px solid #30363d", borderRadius: "16px", padding: "28px", marginBottom: "28px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "16px", flexWrap: "wrap" }}>
            <div style={{ textAlign: "center", flex: 1 }}>
              {team1 && team1.img && <img src={team1.img} alt={team1.name} style={{ width: "52px", height: "52px", borderRadius: "50%", marginBottom: "8px" }} />}
              <div style={{ color: "#e6edf3", fontWeight: "700", fontSize: "16px" }}>{match.teams ? match.teams[0] : ""}</div>
            </div>
            ...
            <div style={{ textAlign: "center", flex: 1 }}>
              {team2 && team2.img && <img src={team2.img} alt={team2.name} style={{ width: "52px", height: "52px", borderRadius: "50%", marginBottom: "8px" }} />}
              <div style={{ color: "#e6edf3", fontWeight: "700", fontSize: "16px" }}>{match.teams ? match.teams[1] : ""}</div>
            </div>
          </div>
          ...
          <div style={{ marginTop: "16px" }}>
            <a href={"https://wa.me/?text=" + shareText} target="_blank" rel="noopener noreferrer" style={{ display: "inline-flex", alignItems: "center", gap: "6px", backgroundColor: "rgba(37,211,102,0.1)", border: "1px solid rgba(37,211,102,0.3)", color: "#25d366", padding: "8px 16px", borderRadius: "8px", fontSize: "13px", fontWeight: "600", textDecoration: "none" }}>
              Share on WhatsApp
            </a>
          </div>
        </div>
        ...
        {match.scorecard && match.scorecard.length > 0 && (
          <div>
            ...
            {match.scorecard.map((inning, index) => (
...
fs.writeFileSync('app/match/[id]/page.tsx', code, 'utf8');
